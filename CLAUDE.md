...
Precedent: simplenotification exposes `GET`/`DELETE /_sakumock/messages` to list and clear accepted notifications; response-validated services expose `GET`/`DELETE /_sakumock/spec-violations` via `core.SpecViolationRoutes` (see "Response validation" below).
...
Control-plane ports are sequential from 18080. Next available: 18095. (18080 simplemq, 18081 kms, 18082 secretmanager, 18083 simplenotification, 18084 monitoringsuite, 18085 eventbus, 18086 objectstorage, 18087 iam, 18088 apprun, 18089 apprundedicated, 18090 workflows, 18091 apigw, 18092 cloudhsm, 18093 seg, 18094 addon.)
...
- Each wired service has a `validate_gen_test.go` consistency test asserting every `bodySchemas` key matches a `routeTable()` entry, so a spec/route drift (e.g. renamed path parameter) fails tests instead of silently disabling validation. **All 15 services are wired.** Services whose routes diverge from spec paths carry a `validate_mapping.json` passed via `-mapping`: secretmanager (`pathRewrites`), simplenotification (`skipPaths` for unimplemented spec paths), objectstorage (`prefix` + per-operation `routes` overrides for the /fed/v1 family). simplemq feeds two specs (`-spec` twice: queue.yaml + message.yaml) into one table and uses two `BodyValidator` instances over it — one per error envelope (data plane vs control plane) — with auth middleware outermost so 401 wins over 400. When adding validation to a new service, follow the monitoringsuite pattern (generate.go, server.go, route.go, Makefile) and delete the superseded hand-written checks.
...
- **Response validation (wired in all 15 services).** Adding `-responses` to the `//go:generate` line makes the same generator also emit a `responseSchemas` table (`map[string]map[int]*core.BodySchema`, route key → declared status → schema) compiled with response semantics: required `writeOnly` properties are skipped instead of `readOnly` ones; a `default` or error-range (`4XX`/`5XX`) response is ignored (undeclared error statuses are tolerated anyway, and dropping the route for a ubiquitous default error catch-all would disable validation of the declared statuses) while a success-range (`2XX`/`3XX`) response degrades the route to permissive; a required name with no property definition alongside it (a spec inconsistency — ogen-generated SDK types drop it too) is skipped with a NOTE in both request and response tables. `core.NewResponseValidator(responseSchemas, logger)` wraps each route **innermost** — inside the request validator — so only what the handler itself writes is checked, never a 429/400 written by an outer middleware. A violation (undeclared success status, non-JSON/empty body where the spec declares one, or schema mismatch) never alters the response the client receives — undeclared **error** statuses (4xx/5xx) are tolerated because specs routinely omit error statuses the real API returns (e.g. 404 for a missing resource), while declared error statuses still get their body validated: it is logged at Warn and recorded deduplicated with a count, exposed via `Server.SpecViolations()` and the inspection routes `GET`/`DELETE /_sakumock/spec-violations` (appended to `routeTable()` with `core.SpecViolationRoutes(s.respValidator)`). This is how the mock detects its own spec drift without breaking clients. Each service's tests assert zero violations — most via a `closeAndCheck` test helper wrapping every `srv.Close()` (or the shared `newServer` helper), so any test that exercises a drifting route fails. Note that `go test` swallows the WARN logs of passing tests, so the assertions are the only reliable signal — when adding tests, close servers through the helper. When wiring a new service, expect the validator to surface latent mismatches (wrong field names, missing required fields, bare arrays where the spec declares a paginated object, error envelopes missing spec-required fields) — fix the handlers rather than weakening the schema.
- **Spec gaps** (constraints the real API enforces but the spec omits) are declared, not hand-coded. The recurring case — a string field that is required but has no `minLength`, while the real API rejects `""` — goes in the service's hand-written `validate_overrides.go` (`var bodyNonEmptyFields map[string][]string`, dotted property paths per route key; a `[]` suffix on a segment steps into array items, e.g. `Remark.Servers[].IPAddress`) and is applied via `core.WithNonEmpty(bodyNonEmptyFields)` on `NewBodyValidator`. The overlay copies schema nodes (never mutates the generated table), a spec-declared minLength wins, and an entry whose route or path no longer resolves panics at construction so a spec rename fails tests loudly. Remove an entry once the upstream spec gains the minLength. Checks that are neither spec-expressible nor a non-empty gap (cross-field comparisons, value allow-lists, reserved ranges, referential lookups) stay hand-written in handlers.
...
- End-to-end Terraform test: `test/terraform/` (root module) drives the real `sakumock all` binary with the `sacloud/sakura` provider through a full apply → plan(no-diff) → destroy for one resource per service. It is behind the `terraform` build tag (so normal `go test ./...` skips it) and `t.Skip`s when the `terraform` binary is absent; run it with `go test -tags terraform ./test/terraform/`. CI runs it in the `terraform-integration` job (fetches the provider from the registry). A new service's resource should be added to `test/terraform/main.tf`.
  - To find out which resources the provider exposes for a service, ask the provider: `terraform -chdir=test/terraform init` once, then `terraform -chdir=test/terraform providers schema -json | jq -r '.provider_schemas[].resource_schemas | keys[]'` (append `["<resource>"].block.attributes` to inspect one resource's arguments). Do **not** grep the provider binary for `sakura_<something>`: terraform-plugin-framework composes the type name at runtime (`req.ProviderTypeName + "_<suffix>"`), so the full name is never a literal in the binary and the grep silently reports nothing for resources that do exist.
  - A failed run leaves `test/terraform/terraform.tfstate` holding the previous mock process's resource IDs, so the next run fails during refresh with unrelated 404s. Delete `terraform.tfstate*` (gitignored) before re-running.
