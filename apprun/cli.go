...
// Command is the CLI command for the AppRun mock server. It embeds Config so the
// same struct works both as a standalone binary (flat flags) and as a
// subcommand of the unified sakumock binary.
type Command struct {
	Config
	TLS    core.TLSFiles `embed:"" prefix:"tls-" envprefix:"APPRUN_TLS_"`
...
func (c *Command) Run(ctx context.Context) error {
	...
	slog.Info("sakumock-apprun starting",
