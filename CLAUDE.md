...
- Each wired service has a `validate_gen_test.go` consistency test asserting every `bodySchemas` key matches a `routeTable()` entry, so a spec/route drift (e.g. renamed path parameter) fails tests instead of silently disabling validation. **All 13 services are wired.** Services whose routes diverge from spec paths carry a `validate_mapping.json` passed via `-mapping`: secretmanager (`pathRewrites`), simplenotification (`skipPaths` for unimplemented spec paths), objectstorage (`prefix` + per-operation `routes` overrides for the /fed/v1 family). simplemq feeds two specs (`-spec` twice: queue.yaml + message.yaml) into one table and uses two `BodyValidator` instances over it — one per error envelope (data plane vs control plane) — with auth middleware outermost so 401 wins over 400. When adding validation to a new service, follow the monitoringsuite pattern (generate.go, server.go, route.go, Makefile) and delete the superseded hand-written checks.
