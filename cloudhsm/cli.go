...
// Command is the CLI command for the CloudHSM mock server. It embeds Config so
// the same struct works both as a standalone binary (flat flags) and as a
// subcommand of the unified sakumock binary.
type Command struct {
	Config
	TLS    core.TLSFiles `embed:"" prefix:"tls-" envprefix:"CLOUDHSM_TLS_"`
...
func (c *Command) Run(ctx context.Context) error {
	...
	slog.Info("sakumock-cloudhsm starting",
