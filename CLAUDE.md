...
- Exporter is **HTTP-only** (`otlptracehttp`); `OTEL_EXPORTER_OTLP_PROTOCOL=grpc` logs a warning and http/protobuf is used. Note `otlptracehttp` transitively links `google.golang.org/grpc`/`grpc-gateway` (never executed) — an accepted trade-off for correct standard-env-var semantics; it does not contradict `monitoringsuite/dataplane.go`'s choice to keep the mock's own *ingest decoding* off the collector service packages.
