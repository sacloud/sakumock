...
// Command is the CLI command for the Add-on mock server. It embeds Config so
// the same struct works both as a standalone binary (flat flags) and as a
// subcommand of the unified sakumock binary.
type Command struct {
	Config
	TLS    core.TLSFiles `embed:"" prefix:"tls-" envprefix:"ADDON_TLS_"`
...
func (c *Command) Run(ctx context.Context) error {
	...
	slog.Info("sakumock-addon starting",
		"version", sakumock.Version,
		"addr", c.Addr,
		"provisioning_delay", c.ProvisioningDelay,
		"latency", c.Latency,
