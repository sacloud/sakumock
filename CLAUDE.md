...
- **Response validation (rolling out service by service — a service is wired iff its `generate.go` passes `-responses`; currently iam, kms, and secretmanager).** Adding `-responses` to the `//go:generate` line makes the same generator also emit a `responseSchemas` table (`map[string]map[int]*core.BodySchema`, route key → declared status → schema) compiled with response semantics: required `writeOnly` properties are skipped instead of `readOnly` ones; a `default` or range (`4XX`) response degrades the route to permissive; a required name with no property definition alongside it (a spec inconsistency — ogen-generated SDK types drop it too) is skipped with a NOTE in both request and response tables. `core.NewResponseValidator(responseSchemas, logger)` wraps each route **innermost** — inside the request validator — so only what the handler itself writes is checked, never a 429/400 written by an outer middleware. A violation (undeclared success status, non-JSON/empty body where the spec declares one, or schema mismatch) never alters the response the client receives — undeclared **error** statuses (4xx/5xx) are tolerated because specs routinely omit error statuses the real API returns (e.g. 404 for a missing resource), while declared error statuses still get their body validated: it is logged at Warn and recorded deduplicated with a count, exposed via `Server.SpecViolations()` and the inspection routes `GET`/`DELETE /_sakumock/spec-violations` (appended to `routeTable()` with `core.SpecViolationRoutes(s.respValidator)`). This is how the mock detects its own spec drift without breaking clients: services can be migrated gradually, and SDK-based tests assert `SpecViolations()` is empty after exercising endpoints so drift on exercised routes fails tests. When wiring a new service, expect the validator to surface latent mismatches (wrong field names, missing required fields, bare arrays where the spec declares a paginated object, error envelopes missing spec-required fields) — fix the handlers rather than weakening the schema.
