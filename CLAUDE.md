...
Precedent: simplenotification exposes `GET`/`DELETE /_sakumock/messages` to list and clear accepted notifications; iam exposes `GET`/`DELETE /_sakumock/spec-violations` to list and clear recorded OpenAPI spec violations (see "Response validation" below).
...
- **Response validation (rolling out; currently wired in iam only).** Adding `-responses` to the `//go:generate` line makes the same generator also emit a `responseSchemas` table (`map[string]map[int]*core.BodySchema`, route key → declared status → schema) compiled with response semantics: required `writeOnly` properties are skipped instead of `readOnly` ones; a `default` or range (`4XX`) response degrades the route to permissive; a required name with no property definition alongside it (a spec inconsistency — ogen-generated SDK types drop it too) is skipped with a NOTE in both request and response tables. `core.NewResponseValidator(responseSchemas, logger)` wraps each route **innermost** — inside the request validator — so only what the handler itself writes is checked, never a 429/400 written by an outer middleware. A violation (undeclared status, non-JSON/empty body where the spec declares one, or schema mismatch) never alters the response the client receives: it is logged at Warn and recorded deduplicated with a count, exposed via `Server.SpecViolations()` and the inspection routes `GET`/`DELETE /_sakumock/spec-violations`. This is how the mock detects its own spec drift without breaking clients: services can be migrated gradually, and SDK-based tests assert `SpecViolations()` is empty after exercising endpoints so drift on exercised routes fails tests. When wiring a new service, expect the validator to surface latent mismatches (wrong field names, missing required fields, bare arrays where the spec declares a paginated object, error envelopes missing spec-required fields) — fix the handlers rather than weakening the schema.
