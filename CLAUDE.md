...
- Each wired service has a `validate_gen_test.go` consistency test asserting every `bodySchemas` key matches a `routeTable()` entry, so a spec/route drift (e.g. renamed path parameter) fails tests instead of silently disabling validation. Currently wired: monitoringsuite, kms, workflows, iam, eventbus, apprundedicated, apprun (the services whose routes mirror the spec paths verbatim). Not yet wired: secretmanager, objectstorage, simplemq, simplenotification — their routes diverge from spec paths and need a `-mapping` file. When adding validation to another service, follow the monitoringsuite pattern (generate.go, server.go, route.go, Makefile) and delete the superseded hand-written checks.
